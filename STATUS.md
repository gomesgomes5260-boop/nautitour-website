...
Última atualização: **12/ago/2026** — PR de correções das páginas públicas (export do Notion #2): **sistema de tags de imagens** no `/admin/imagens` (migration 043 `site_image_tags` + seed) com galerias das 4 páginas públicas (home/escuna/lancha/locação) vindas do banco por tag `galeria-*` (fallback = arrays estáticos antigos); Home com foto aérea da escuna na CTA + degradê maré no "Como funciona"; Escuna com seção Roteiro (paradas João Fernandes → Ilha Feia → Tartaruga, folder oficial) + aviso de taxa de embarque por píer; Lancha com hero sem overlay vermelho (foto à direita) + CTAs verdes de WhatsApp no hero e aside; Sobre nós redesenhada (hero, fatos, equipe, diferenciais, selos extraídos em `CertificationBadges`, mapa) mantendo o copy real; Locação Privativa redesenhada (hero, ocasiões, passos, form sticky restylizado — lógica intocada). galeria-lancha estreia só com as 2 fotos reais de lancha; o dono tagueia mais pelo admin.

Estado anterior (**17/jul/2026**) — **⚠️ Decisão pós-sessão: canal SMS (PRs #106-#108) e WhatsApp Cloud (PR #104) foram REMOVIDOS — notificação é e-mail-only (migration 033 reverteu a 032; código recuperável no histórico git).** Sessão 12-17/jul (PRs #95-#108, todas mergeadas): biblioteca de imagens `/admin/imagens` (Supabase Storage, otimização client-side WebP, upload de pastas, barra de progresso, aba Todas + ordenação por uso — migrations 030-031); e-mails de cancelamento (cliente 48h + saída por clima/Marinha) e rebrand dos 5 templates (logo branca + verde/azul/vermelho por categoria); hardening de pentest (rate limit por conta + alerta Sentry, anti-enumeração no signup, sanitize PostgREST); WhatsApp canônico do site trocado pra (22) 99246-7880 com hardcodes centralizados; e **canal SMS via Comtele** (PRs #106-#108, migration 032) pra lembrete D-1 + mudança de horário + cancelamento de saída — mensagens ≤160 chars sem acento, idempotência por canal, `booking_events sms_sent`; WhatsApp Cloud API adormecido (`whatsapp-cloud.ts`, Meta travou onboarding). Pendente: env `COMTELE_API_KEY` no Vercel + SMS de teste real.
