...
- **Modo Pagar.me `live`** desde 11/maio. PIX funcionando em produção desde o go-live. **Cartão só passou a funcionar em 12/ago** (antes falhava 100% com `validation_error | billing | "value" is required`): a antifraude da Stone exige `billing_address` no pedido de cartão, e nós não enviávamos nenhum endereço. Fix: checkout de cartão coleta CEP+número (autofill ViaCEP) e envia `credit_card.card.billing_address`. Recusas de cartão agora viram `booking_event` (`payment_failed`) — antes o caminho síncrono do cartão não logava nada.
...
- **Lancha privativa SEM checkout online** (decisão 05/ago): o cliente escolhe data/hora no calendário e cai no WhatsApp (`/api/wa?s=lancha-data&dt=...`) pra consultar disponibilidade; pagamento na loja. Checkout 404 pra `tour_type='private'` (+ guard na action). Reserva entra MANUAL pelo `/vendedor` — RPC `seller_create_booking` aceita private desde a migration 037 (preço fixo do slot, saída vira sold_out)
...
- **Galeria de fotos**: `PhotoGallery` em home + 3 tour pages com lightbox custom. Desde 12/ago as galerias vêm do banco por TAG (`site_image_tags`, migration 043): admin tagueia a foto com `galeria-home`/`galeria-escuna`/`galeria-lancha`/`galeria-locacao` em `/admin/imagens` e ela entra na galeria da página (ordem = ordem de tagueamento). Loader `src/lib/gallery.ts` (`getGalleryPhotos(tag, fallback)`); os arrays estáticos de `src/lib/photo-gallery.ts` viraram FALLBACK (não deletar `/public/images/photos`). Tags livres (lancha, escuna, bar, drone-*) organizam a biblioteca; sugestões em `src/lib/image-tags.ts`. galeria-lancha começou só com as 2 fotos reais de lancha (`misc/seq-0002*`) — o dono tagueia mais.
